 ...
 * Class UpdateTest
 *
 * @covers \rwt\SwiftPDO\Queries\Update
 */
class UpdateTest extends TestCase
{
    ...
    {
        $query = $this->swift->update('country')->set('name', 'aikavolS')->where('id', 1);
        $query->execute();

        $query2 = $this->swift->from('country')->where('id', 1);
        ...
        $this->swift->update('country')->set('name', 'Slovakia')->where('id', 1)->execute();
        $query3 = $this->swift->from('country')->where('id', 1);
    ...
    {
        $query = $this->swift->update('article')->set('published_at', new RWT\SwiftPDO\Literal('NOW()'))->where('user_id', 1);
    ...
    {
        $query = $this->swift->update('user')->set(['name' => 'keraM', '`type`' => 'author'])->where('id', 1);
    ...
    {
        $query = $this->swift->update('user', ['type' => 'admin'], 1);
    ...
    {
        $this->swift->update('article')->set('content', '')->where('id', 1)->execute();
        $user = $this->swift->from('article')->where('id', 1)->fetch();

        $printQuery = "ID: {$user['id']} - content: {$user['content']}";

        $this->swift->update('article')->set('content', 'content 1')->where('id', 1)->execute();

        $user2 = $this->swift->from('article')->where('id', 1)->fetch();
    ...
    {
        $query = $this->swift->update('users')
            ->set("`users`.`active`", 1)
            ->where("`country`.`name`", 'Slovakia')
            ->where("`users`.`name`", 'Marek');

        $query2 = $this->swift->update('users')
            ->set("[users].[active]", 1)
            ->where("[country].[name]", 'Slovakia')
            ->where("[users].[name]", 'Marek');
