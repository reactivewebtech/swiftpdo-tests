 ...
 * Class SelectTest
 *
 * @covers \rwt\SwiftPDO\Queries\Select
 */
class SelectTest extends TestCase
{
    ...
    {
        $query = $this->swift->from('user author')->getQuery(false);
        $query2 = $this->swift->from('user AS author')->getQuery(false);
        $query3 = $this->swift->from('user AS author', 1)->getQuery(false);
        $query4 = $this->swift->from('user AS author')->select('country.name')->getQuery(false);
    ...
    {
        $query = $this->swift->from('article')->where('NOT id', [1, 2]);
    ...
    {
        $query = $this->swift->from('article')->group('user_id')->order('id');
    ...
    {
        $queryPrint = $this->swift->from('user', 1)->fetch('name');
        $queryPrint2 = $this->swift->from('user', 1)->fetch();
        $statement = $this->swift->from('user', 5)->fetch();
        $statement2 = $this->swift->from('user', 5)->fetch('name');
    ...
    {
        $result = $this->swift->from('user')->fetchPairs('id', 'name');
        $result2 = $this->swift->from('user')->fetchAll();
    ...
    {
        $result = $this->swift->from('user')->fetchAll('id', 'type, name');
    ...
    {
        $printColumn = $this->swift->from('user', 3)->fetchColumn();
        $printColumn2 = $this->swift->from('user', 3)->fetchColumn(3);
        $statement = $this->swift->from('user', 5)->fetchColumn();
        $statement2 = $this->swift->from('user', 5)->fetchColumn(3);
