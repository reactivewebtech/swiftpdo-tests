...
use RWT\SwiftTest\Model\User;
use RWT\SwiftPDO\Query;

/**
 * Class CommonTest
 *
 * @covers \rwt\SwiftPDO\Queries\Common
 */
class CommonTest extends TestCase
{
    ...
    public function testShortJoin()
    {
        ...
        $query = $this->swift->from('article')->leftJoin('user');
        $query2 = $this->swift->from('article')->leftJoin('user author');
        $query3 = $this->swift->from('article')->leftJoin('user AS author');
    ...
    {
        $query = $this->swift->from('user')->innerJoin('article:');
        $query2 = $this->swift->from('user')->innerJoin('article: with_articles');
        $query3 = $this->swift->from('user')->innerJoin('article: AS with_articles');
    ...
    {
        $query = $this->swift->from('article')->where('comment:content <> "" AND user.country.id = ?', 1);
    ...
    {
        $query = $this->swift->from('article')->select('user.name AS author');
    ...
    {
        $query = $this->swift->from('article')->orderBy('user.name, article.title');
    ...
    {
        $query = $this->swift->from('article')->groupBy('user.type')
    ...
    public function testDontCreateDuplicateJoins()
    {
        ...
        $query2 = $this->swift->from('article')
            ->innerJoin('user ON article.user_id = user.id')
            ->select('user.name');

        $query3 = $this->swift->from('article')
            ->innerJoin('user AS author ON article.user_id = author.id')
            ->select('author.country.name');

        $query4 = $this->swift->from('article')
            ->innerJoin('user ON article.user_id = user.id')
    ...
    {
        $query = $this->swift->from('article')->select('user.name')->innerJoin('user');
        $query2 = $this->swift->from('article')->select('author.name')->innerJoin('user AS author');
        $query3 = $this->swift->from('user')->select('article:title')->innerJoin('article:');
    ...
    {
        $queryPrint = $this->swift->from('db2.user')->order('db2.user.name')->getQuery(false);
    ...
    public function testDisableSmartJoin()
    {
        ...
        $query2 = $this->swift->from('comment')
            ->select('user.name')
            ->disableSmartJoin()
            ->orderBy('article.published_at')
            ->getQuery(false);
        ...
        $query3 = $this->swift->from('comment')
            ->disableSmartJoin()
            ->select('user.name')
            ->enableSmartJoin()
            ->orderBy('article.published_at')
    ...
    {
        $query = $this->swift->from('user')->where('id > ?', 0)->orderBy('name');
        $query = $query->where('name = ?', 'Marek');
        $this->swift->getPdo()->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_OBJ);
    ...
    {
        $query = $this->swift->from('user', 2)->asObject();
    ...
    public function testFromIdAsObjectUser()
    {
        ...
        $query = $this->swift->from('user', 2)->asObject(User::class);
        $user = $query->fetch();
